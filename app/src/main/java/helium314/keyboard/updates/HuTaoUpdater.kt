...
object HuTaoUpdater {
    private const val TAG = "HuTaoUpdater"
    private const val MANIFEST_URL =
        "https://github.com/Twindisme/HuTaoBoard/releases/latest/download/update.json"
    private const val RELEASE_PATH_PREFIX = "/Twindisme/HuTaoBoard/releases/download/"
    private const val PREFS_NAME = "hu_tao_updates"
    ...
    private const val CHANNEL_ID = "hu_tao_updates"
    private const val NOTIFICATION_ID = 0x4854
    ...
    fun getAvailableUpdate(context: Context): HuTaoUpdateInfo? {
        ...
        return HuTaoUpdateInfo(versionCode, versionName, apkUrl, sha256)
    }

    fun downloadAndVerify(context: Context, update: HuTaoUpdateInfo): File {
        require(isTrustedApkUrl(update.apkUrl)) { "Untrusted update URL" }
        val updateDir = File(context.cacheDir, "updates").apply { mkdirs() }
        val partialFile = File(updateDir, "HuTaoBoard-update.apk.part")
        val apkFile = File(updateDir, "HuTaoBoard-update.apk")
        partialFile.delete()
        apkFile.delete()
    ...
    private fun userAgent() = "HuTaoBoard/${BuildConfig.VERSION_NAME}"

    @SuppressLint("MissingPermission")
    private fun showUpdateNotification(context: Context, update: HuTaoUpdateInfo) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            manager.createNotificationChannel(
                NotificationChannel(
                    CHANNEL_ID,
                    context.getString(R.string.hu_tao_update_channel),
                    NotificationManager.IMPORTANCE_DEFAULT,
                ),
            )
        }
        val intent = Intent(context, HuTaoUpdateActivity::class.java)
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            .setSmallIcon(R.drawable.hu_tao_update_notification)
            .setContentTitle(context.getString(R.string.hu_tao_update_available_title))
            .setContentText(context.getString(R.string.hu_tao_update_available_text, update.versionName))
